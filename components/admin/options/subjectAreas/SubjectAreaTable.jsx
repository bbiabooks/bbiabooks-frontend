...
const SubjectAreaTable = ({
    ...
    const filteredSubjectAreas = subjectAreas.filter((subjectAreaV) => {
        const { subjectArea } = subjectAreaV;

        return subjectArea.toLowerCase().includes(searchTerm.toLowerCase());
    });
                    ...
                    <div className="flex flex-col items-end space-x-2 text-cyan-600 opacity-70 hover:opacity-100 mb-4">
                        <div className="flex justify-end">
                            <p className="text-sm font-bold text-gray-600">
                                Total Subject Areas: { filteredSubjectAreas.length }
                            </p>
                        </div>
